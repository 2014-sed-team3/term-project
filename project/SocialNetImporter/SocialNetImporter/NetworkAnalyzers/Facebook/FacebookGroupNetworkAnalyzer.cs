...
namespace Smrf.NodeXL.GraphDataProviders.Facebook
{
    ...
    public class FacebookGroupNetworkAnalyzer : FacebookNetworkAnalyzerBase
    {
        ...
        {            
            ...
            oGetNetworkAsyncArgs.groupID = sGroupID;
        ...
        )
        {
            ...
            return (GetFanPageNetworkInternal(s_accessToken, sGroupID,
                                                netTypes, iFromPost,
                                                iToPost,
...

        {
            ...
            //Download data
            streamPosts = DownloadPosts(sGroupID, iFromPost, iToPost, includeOthers, startDate, endDate);
            ...
            {
                commentersComments = DownloadComments(streamPosts);
            }
            ...
            {
                likersPosts = DownloadLikes(streamPosts);
            }
            ...
            OnNetworkObtainedWithoutTerminatingException(oGraphMLXmlDocument, oRequestStatistics,
                GetNetworkDescription(streamPosts, sGroupID, netTypes,
        ...
        )
        {
            ...
            JSONObject streamPosts = null;
            List<JSONObject> resultPosts = new List<JSONObject>();

            CurrentStep++;
            ReportProgress(String.Format("Step {0}/{1}: Downloading Fan Page Data", CurrentStep, NrOfSteps));

            //Get group details from Facebook
            JSONObject fanPage = ExecuteFQLWithRelogin("SELECT gid, name, description, pic_square FROM group " +
                                                    "WHERE gid = '" + sGroupID + "'",false);
            ...
            if (iFromPost > 0 && iToPost > 0)
            {
                ...
                streamPosts = ExecuteFQLWithRelogin("SELECT message, post_id, comments, likes, created_time, permalink, attachment "
                                                    +"FROM stream WHERE source_id='" + sGroupID + "' LIMIT "+(iFromPost-1)+", " + (iToPost - (iFromPost-1)),true);
                resultPosts.AddRange(streamPosts.Array);
            }
            else
            {
                int nrOfPostsPerCall = 500;
                int start = 0;
                bool hasResults = true;
                ReportProgress(String.Format("Step {0}/{1}: Downloading posts between {2} and {3}", CurrentStep, NrOfSteps, startDate, endDate));
                while (hasResults)
                {
                    streamPosts = ExecuteFQLWithRelogin("SELECT message, post_id, comments, likes, created_time, permalink, attachment FROM stream WHERE source_id='"
                                                + sGroupID + "' AND created_time>=" + ConvertToTimestamp(startDate).ToString() +
                                                " AND created_time<=" + ConvertToTimestamp(endDate).ToString()+" LIMIT "+start+", "+nrOfPostsPerCall,true);
                    start += nrOfPostsPerCall;

                    //In case we get an error or less than nrOfPostsPerCall 
                    //results we stop downloading posts
                    if (streamPosts == null ||
                        !streamPosts.IsArray ||
                        streamPosts.Array.Length < nrOfPostsPerCall)
                    {
                        hasResults = false;
                    }
                    resultPosts.AddRange(streamPosts.Array);
        ...
        )
        {
            ...
            for (int i = 0; i < streamPosts.Count; i++)
            {
                ...
                if (!streamPosts[i].Dictionary["likes"].Dictionary.ContainsKey("count")) continue;
                int totalNrOfLikes = (int)streamPosts[i].Dictionary["likes"].Dictionary["count"].Integer;
                ...
                for (int j = 0; j < nrOfMultiqueries; j++)
                {
                    ...
                    {
                        queries.Add("query" + (k + 1).ToString(), "SELECT user_id, post_id FROM like WHERE post_id='" + streamPosts[i].Dictionary["post_id"].String + "' LIMIT " + (k * 1000).ToString() + ", 1000");                            
                    }

                    result = ExecuteFQLMultiqueryWithRetryRelogin(queries, String.Format("Step {0}/{1}: Downloading likes - Post {2}/{3}(Batch {4}/{5})",
                                                    CurrentStep, NrOfSteps, i + 1, streamPosts.Count, j + 1, nrOfMultiqueries),false);
        ...
        )
        {
            ...
            {
                //Get the total number of likes for the actual post
                int totalNrOfComments = (int)streamPosts[i].Dictionary["comments"].Dictionary["count"].Integer;
                //We can get a maximum of 10000 results for multiqueries, so we have to make multiple multiqueries
                //We have to get information from two different tables and we are limited to 5000 results each
                //for a total of 10000 results for the whole multiquery
                ...
                for (int j = 0; j < nrOfMultiqueries; j++)
                {
                    ...
                    for (int k = 0; k < nrOfQueries; k++)
                    {
                        queries.Add("query" + (k + 1).ToString(), "SELECT fromid, text, post_id FROM comment WHERE post_id='" + streamPosts[i].Dictionary["post_id"].String + "' ORDER BY time ASC LIMIT " + (k * 1000).ToString() + ", 1000");
                    }
        ...
        )
        {
            ...
            //Add nodes (posts)
            foreach (JSONObject ob in streamPosts)
            {
        ...
        )
        {
            ...
            foreach(JSONObject item in streamPosts)
            {
                ...
                for (int i = 0; i < kvp.Value.Count - 1; i++)
                {
                    for (int j = i + 1; j < kvp.Value.Count; j++)
                    {
                        ...
                        string[] postsToJoin = (from p in postsIntersection
                                                        select new { Post = streamPosts.Single(x => x.Dictionary["post_id"].String.Equals(p)).Dictionary["message"].String }).Select(x => x.Post).ToArray();
                        posts = String.Join("\n\n", postsToJoin);
        ...
        )
        {
            ...
            foreach (KeyValuePair<string, List<string>> kvp in postLikers)
            {
                ...
                {
                    //Sometimes a commenter or liker can be another fan page and this is not a user
                    if (!usersDisplayName.ContainsKey(kvp.Value[i])) continue;
                    for (int j = i + 1; j < kvp.Value.Count; j++)
                    {
                        //Sometimes a commenter or liker can be another fan page and this is not a user
                        if (!usersDisplayName.ContainsKey(kvp.Value[j])) continue;
                        ...
                        //                             select new { Post = streamPosts.Single(x => x.Dictionary["post_id"].String.Equals(p)).Dictionary["message"].String }).Select(x => x.Post).ToArray());
                        oGraphMLXmlDocument.AppendGraphMLAttributeValue(oEdgeXmlNode, "post", streamPosts.Single(x => x.Dictionary["post_id"].String.Equals(kvp.Key)).Dictionary["message"].String);
        ...
        )
        {
            ...
            if (wallPosts.ContainsKey(userID))
            {
                ...
                {
                    if (((JSONObject)en.Current["message"]).String.Equals("")) continue;
                    currentWallPost += ((JSONObject)en.Current["name"]).String + ": " + ((JSONObject)en.Current["message"]).String + "\n\n";
                    //Add tags
                    if (en.Current["message_tags"] is List<JSONObject>)
                    {
                        foreach (JSONObject ob in (List<JSONObject>)en.Current["message_tags"])
        ...
        )
        {
            ...
            {
                oNetworkDescriber.AddSentence(
                    "Wall posts from {0} to {1} of the fan page are analyzed.",
                    iFromPost, iToPost
                    );
            }
            else
        ...
        )
        {
            ...
            for (int i = 0; i < streamPosts.Count; i++)
            {
                DateTime tmp = ConvertToDateTime(double.Parse(streamPosts[i].Dictionary["created_time"].String));                    
        ...
        {
            FacebookLoginDialog o_fcbLoginDialog = new FacebookLoginDialog(new FacebookGraphDataProviderDialogBase(), UserAttributes.createRequiredPermissionsString(bGetStatusUpdates, bGetWallPosts, true));
